...
namespace Azul
{
	GraphicsObject_Sprite::GraphicsObject_Sprite(Model* pModel, ShaderObject* pShaderObj, Image* _pImage, Rect rect)
		: GraphicsObject(pModel, pShaderObj)
	{
		...
		float w = pImage->imageRect.width / (float)pImage->pTextNode->width;
		float h = pImage->imageRect.height / (float)pImage->pTextNode->height;
		float u = pImage->imageRect.x / (float)pImage->pTextNode->width;
		float v = pImage->imageRect.y / (float)pImage->pTextNode->height;
	...
	void GraphicsObject_Sprite::SetState()
	{
		...
		// Bind the texture
		GLuint textureID = this->pImage->pTextNode->textureID;
