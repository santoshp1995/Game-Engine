...
namespace Azul
{
	...
	WarbearModel::WarbearModel(const char* const pModelFileName, const int index)
		: Model()
	{
	...
	void WarbearModel::CreateVAO(const char* const pModelFileName)
	{
		// chunk shit
		bool status;
		unsigned char* buff;
		...
		if (strcmp(this->texture, "polarbear.tga") == 0)
		{
			Texture::Add(this->texture, TextureName::POLARBEAR);
		}

		if (strcmp(this->texture, "warbear.tga") == 0)
		{
			Texture::Add(this->texture, TextureName::WARBEAR);
		}
	...
	void WarbearModel::GetTextureName(const int index, char*& outTextName)
	{
		switch (index)
		{
		case -1:
		...
		case 0:
			outTextName = "warbear.tga";
			break;

		case 1:
			outTextName = "polarbear.tga";
			break;
