...
namespace Azul
{
	...
	void SpaceFrigateModel::CreateVAO(const char* const pModelFileName)
	{
		// chunk shit
		bool status;
		unsigned char* buff;
		...
		// load the verts data from the .azul file
		...
		if (strcmp(this->texture, "space_frigate.tga") == 0)
		{
			Texture::Add(this->texture, TextureName::SPACE_FRIGATE);
		}
	...
	void SpaceFrigateModel::GetTextureName(const int index, char*& outTextName)
	{
		switch (index)
		{
		case -1:
	...
	SpaceFrigateModel::SpaceFrigateModel(const char* const pModelFileName, int index)
		: Model()
	{
	...
	SpaceFrigateModel::~SpaceFrigateModel()
	{
