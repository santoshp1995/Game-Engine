...
namespace Azul
{
	...
	MilitaryTruckModel::MilitaryTruckModel(const char* const pModelFileName, const int index)
	{
	...
	MilitaryTruckModel::~MilitaryTruckModel()
	{

	}

	void MilitaryTruckModel::CreateVAO(const char* const pModelFileName)
	{
		// chunk shit
		bool status;
		unsigned char* buff;
		...
		Texture::Add(this->texture, TextureName::MILITARY_TRUCK);
	...
	void MilitaryTruckModel::GetTextureName(const int index, char*& outTextName)
	{
		switch (index)
		{
		case -1:
