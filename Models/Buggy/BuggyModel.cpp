...
namespace Azul
{
	...
	BuggyModel::BuggyModel(const char* const pModelFileName, int index)
		: Model()
	{
	...
	void BuggyModel::CreateVAO(const char* const pModelFileName)
	{
		// chunk shit
		bool status;
		unsigned char* buff;
		...
		Texture::Add(this->texture, TextureName::BUGGY);
	...
	void BuggyModel::GetTextureName(const int index, char*& outTextName)
	{
		switch (index)
		{
		case -1:
